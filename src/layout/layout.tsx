...
const menuSections = {
  main: [
    {
      name: "menu.createApplication",
      icon: RiFileAddFill,
      href: "/application-list",
    },
    {
      name: "menu.archiveApplication",
      icon: RiFileAddFill,
      href: "/archive-application-list",
    },
  ],
  services: [
    {
      name: "menu.workingServices",
      icon: RiServiceFill,
      href: "/working-services",
    },
    {
      name: "menu.keepingServices",
      icon: RiUserFill,
      href: "/keeping-services",
    },
  ],
  settings: [
      ...
      <div className="fixed top-4 left-0 z-50 flex items-center gap-3">
        ...
        {isMenuOpen && (
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl text-white font-bold  dark:text-white">
              Cargo-Calc  
            </h1>
          </div>
        )}
      ...
      <div className={`fixed top-0 right-0 ${
        ...
        <select
          ...
          className="rounded-md border border-gray-300 dark:border-gray-600 
            px-3 py-2 bg-white dark:bg-[#2a2167] text-gray-900 dark:text-white
            focus:outline-none focus:ring-1 focus:ring-[#6C5DD3]"
        >
          {languages.map((lang) => (
            <option key={lang.code} value={lang.code} 
              className="bg-white dark:bg-[#2a2167] text-gray-900 dark:text-white">
              {lang.name}
            </option>
          ))}
        </select>
        ...
        <button
          ...
          className="p-2 rounded-lg hover:bg-[#2a2167]"
        >
          {isDarkMode ? (
        ...
        <button
          ...
          className="p-2 rounded-lg hover:bg-[#2a2167] text-white flex items-center gap-2"
          title={t("menu.logout")}
        >
                ...
                {isMenuOpen ? (
                  <>
                    {openSections.includes(section) ? (
                      <ChevronDown className="h-4 w-4 text-gray-100 dark:text-white" />
                    ) : (
                      <ChevronRight className="h-4 w-4 text-gray-100 dark:text-white" />
                    )}
                    <span className="capitalize text-gray-100 dark:text-white">{t(`menu.${section}`)}</span>
                  </>
                ) : (
                  <ChevronRight className="h-4 w-4 text-gray-100 dark:text-white" />
                )}
