...
export default function CreateKeepingService() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // First API call to create the service
      const serviceResponse = await apiService.createKeepingService({
        name: formData.name
      });

      // Second API call to update with pricing details
      await apiService.updateKeepingService(serviceResponse.data.id, {
        name: formData.name,
        year: formData.year,
  ...
  return (
    <FormLayout
      ...
      subtitle={t('createKeepingService.subtitle', 'Add a new keeping service with pricing details')}
    >
        ...
        <div className="dark:bg-gray-800 p-4 sm:p-6 lg:p-8 rounded-lg shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div className="col-span-1 sm:col-span-2">
              ...
              >
                {t('createKeepingService.name', 'Service Name')}
              </label>
              <input
                type="text"
                name="name"
